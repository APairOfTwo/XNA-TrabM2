// Author: abi
//         com pequenas alterações de Alexandre Lobão
...
using Microsoft.Xna.Framework.Graphics;  //  para usar Graphics Adapter
using System;
...
namespace XNA_TrabM2
{
	...
	class Input
	{
		...
		internal static void Update(GameTime gameTime)
		{
			...
			if (mouseWasCentered) //Basegame.IsWindowActive)
			{
				mouseWasCentered = false;
                MousePos = new Point(GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Width / 2, GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Height / 2);
				mouseState = Microsoft.Xna.Framework.Input.Mouse.GetState();
			} // if
			...
			{
				if (leftRumble > 0)
                    leftRumble -= 0.85f * (float)gameTime.ElapsedGameTime.TotalMilliseconds;
				if (rightRumble > 0)
                    rightRumble -= 0.85f * (float)gameTime.ElapsedGameTime.TotalMilliseconds;
				if (leftRumble < 0)
					leftRumble = 0;
				if (rightRumble < 0)
					rightRumble = 0;
